...
list< string > 
StringTools::split( string source, char sep, unsigned int num )
{
  // FIXME could be more performant
  string ssep;
  ssep+=sep;
  return split ( source, ssep, num );
}

pair < string, string >
  StringTools::splitPath ( const string & path )
{
  list<string> paths = split( path, string(":"), 1 );
  if( paths.size() == 2 )
    return pair < string, string > ( paths.front(), paths.back() );
  else
    return pair < string, string > ( paths.front(), "" );
}
